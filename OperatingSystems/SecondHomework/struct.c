...
void printUsers (PersonHandler* p)
{
    for (int i = 0; i < p->size; ++i)
    {
            printf("First name: %s, Last name: %s, Birt year: %d, Phone number: %s, Paid: %s, Vaccinated: %s\n", p->persons[i].firstName, p->persons[i].lastName, p->persons[i].birthYear, p->persons[i].phoneNumber, p->persons[i].isWillingToPay == 0 ? "No" : "Yes", p->persons[i].vaccinated == 0 ? "No" : "Yes");
    }
}
...
{
    printf("First name: %s, Last name: %s, Birt year: %d, Phone number: %s, Paid: %s, Vaccinated: %s\n", p->firstName, p->lastName, p->birthYear, p->phoneNumber, p->isWillingToPay == 0 ? "No" : "Yes", p->vaccinated == 0 ? "No" : "Yes");
}
